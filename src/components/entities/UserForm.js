...
export default function UserForm({onSubmit,onCancel, initialUser=emptyUser}){
    ...
    const validation = {
       isValid: { 
        ...
        STREET: (name) => name.length > 5,
        CITY: (name) => name.length > 2,
        POSTCODE: (postcode) => /[A-Z]{1,2}[0-9]{1,2}[A-Z]?\s?[0-9][A-Z]{2}/i.test(postcode),
        EMAIL: (email) => /^[a-zA-Z0-9]+(?:\.[a-zA-Z0-9]+)*@[a-zA-Z0-9]+(?:\.[a-zA-Z0-9]+)*$/.test(email),
        PHONENO: (phone) => /^(\+44)[1-9]\d{9}$/.test(phone)
